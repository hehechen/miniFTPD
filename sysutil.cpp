...
int accept_timeout(int fd, struct sockaddr_in *addr, unsigned int wait_seconds)
{
	...
	if (wait_seconds > 0)
	{
		...
		timeout.tv_usec = 0;
		do
		{//socket是一个监听socket,并且已经完成的连接数为非0.这样的soocket处于可读状态
	...
	FTPD_LOG(DEBUG,"accept listenfd:%d",fd);
	if (addr != NULL)//当已连接队列非空，accept不会阻塞
